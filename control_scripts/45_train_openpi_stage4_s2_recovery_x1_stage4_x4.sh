...
#
# Monitor:
#   bash control_scripts/45_train_openpi_stage4_s2_recovery_x1_stage4_x4.sh --status
...
show_status() {
  ...
  ps -ef \
    | grep -E 'compute_norm_stats.py|scripts/train.py|45_train_openpi_stage4_s2_recovery_x1_stage4_x4' \
    | grep -v grep \
    | grep -v -- '--status' || true
  echo
...
stop_training() {
  ...
  pkill -TERM -f "45_train_openpi_stage4_s2_recovery_x1_stage4_x4.sh" 2>/dev/null || true
  sleep 2
  show_status
}
...
echo "Status: bash control_scripts/45_train_openpi_stage4_s2_recovery_x1_stage4_x4.sh --status"
