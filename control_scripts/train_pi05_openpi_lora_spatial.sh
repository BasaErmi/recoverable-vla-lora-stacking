...
CONFIG_NAME="${CONFIG_NAME:-pi05_so101_cuhksz_slots_spatial_lora}"
DATASET_ID="${DATASET_ID:-guanlin8/cuhksz_pick_place_slots_ordered_spatial_20260502}"
LOG_DIR="${LOG_DIR:-/home/ubuntu/outputs/openpi/logs}"
RUN_PREFIX="${RUN_PREFIX:-pi05_so101_cuhksz_slots_spatial_lora_$(date +%Y%m%d_%H%M%S)}"
...
echo "=== OpenPI pi0.5 SO101 spatial-prompt LoRA training manager ==="
...
for BATCH_SIZE in ${BATCH_SIZES:-2}; do
