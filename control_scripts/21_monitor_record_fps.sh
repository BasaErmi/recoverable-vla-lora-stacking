#!/bin/bash
# Monitor SO101 data-collection loop frequency from the metrics emitted by
# control_scripts/09_record_data.sh.
#
# Usage:
#   bash control_scripts/21_monitor_record_fps.sh
#   bash control_scripts/21_monitor_record_fps.sh outputs/record_logs/latest
#   WINDOW_N=180 WARN_BELOW_HZ=28 BAD_BELOW_HZ=24 bash control_scripts/21_monitor_record_fps.sh
...
LOG_ROOT="${LOG_ROOT:-$ROOT_DIR/outputs/record_logs}"
TARGET="${1:-}"
...
if [ -z "$TARGET" ]; then
    echo "ERROR: no record log found. Start data collection first:" >&2
    echo "  bash control_scripts/09_record_data.sh ..." >&2
    exit 1
fi
...
if [ -f "$TARGET" ]; then
    ...
    METRICS_LOG="$TARGET/record_metrics.csv"
fi

echo "Metrics log: $METRICS_LOG"
echo "Window: ${WINDOW_N:-120} samples  Warn: <${WARN_BELOW_HZ:-28}Hz  Bad: <${BAD_BELOW_HZ:-24}Hz"
echo ""

METRICS_LOG="$METRICS_LOG" \
WINDOW_N="${WINDOW_N:-120}" \
PRINT_EVERY_S="${PRINT_EVERY_S:-1.0}" \
WARN_BELOW_HZ="${WARN_BELOW_HZ:-28}" \
BAD_BELOW_HZ="${BAD_BELOW_HZ:-24}" \
...
def print_stats(samples: list[dict[str, str]], total: int) -> None:
    ...
    datasets = [row_float(row, "dataset_ms") for row in samples]
    ...
    print(
        f"phase={phase} t={fmt(elapsed, 's')} total={total} win={len(samples)} "
        ...
        f"dataset_ms p90={fmt(percentile(datasets, 0.90))} "
    ...
    while True:
        ...
        if not line:
            if time.monotonic() - last_data > 2.0:
                print("waiting for new record samples...", flush=True)
                last_data = time.monotonic()
            time.sleep(0.1)
            continue
