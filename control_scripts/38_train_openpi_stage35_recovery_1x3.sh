...
# OpenPI pi0.5 CUHK Stage 3.5 recovery-heavy continuation training.
#
# Default run:
#   bash control_scripts/38_train_openpi_stage35_recovery_1x3.sh
#
# Monitor:
#   bash control_scripts/38_train_openpi_stage35_recovery_1x3.sh --status

OPENPI_DIR="${OPENPI_DIR:-/home/ubuntu/openpi}"
PYTHON_BIN="${PYTHON_BIN:-${OPENPI_DIR}/.venv/bin/python}"
CONFIG_NAME="${CONFIG_NAME:-pi05_so101_cuhksz_stage35_recovery_1x3_lora}"
DATASET_ID="${DATASET_ID:-guanlin8/cuhksz_pi05_stage35_recovery_1x3_20260505}"
LOG_DIR="${LOG_DIR:-/home/ubuntu/outputs/openpi/logs}"
RUN_PREFIX="${RUN_PREFIX:-pi05_so101_stage35_recovery_1x3_20260505_$(date +%Y%m%d_%H%M%S)_from_stage3_45k_lr3e-6}"
...
latest_stage35_log() {
  find "$LOG_DIR" -maxdepth 1 -type f \
    -name 'pi05_so101_stage35_recovery_1x3_20260505_*_from_stage3_45k_lr3e-6*_b*.log' \
    -printf '%T@ %p\n' 2>/dev/null | sort -nr | awk 'NR==1 {print $2}'
}

latest_manager_pid_file() {
  find "$LOG_DIR" -maxdepth 1 -type f \
    -name 'pi05_so101_stage35_recovery_1x3_20260505_*_from_stage3_45k_lr3e-6*_manager.pid' \
    -printf '%T@ %p\n' 2>/dev/null | sort -nr | awk 'NR==1 {print $2}'
}

show_status() {
  echo "=== OpenPI Stage 3.5 manager ==="
  ...
  if [[ -n "${manager_pid_file:-}" ]]; then
    ...
    echo "No Stage 3.5 manager pid file found in $LOG_DIR"
  fi
  echo
  echo "=== OpenPI Stage 3.5 training processes ==="
  ps -ef \
    | grep -E 'compute_norm_stats.py|scripts/train.py|38_train_openpi_stage35_recovery_1x3' \
    | grep -v grep \
    | grep -v -- '--status' || true
  echo
  echo "=== Latest Stage 3.5 train log ==="
  local latest_log
  latest_log="$(latest_stage35_log || true)"
  if [[ -z "${latest_log:-}" ]]; then
    echo "No Stage 3.5 train log found in $LOG_DIR"
    return 0
  fi
  echo "$latest_log"
...
stop_training() {
  echo "Stopping OpenPI Stage 3.5 training processes..."
  pkill -TERM -f "scripts/train.py ${CONFIG_NAME}" 2>/dev/null || true
  pkill -TERM -f "38_train_openpi_stage35_recovery_1x3.sh" 2>/dev/null || true
  sleep 2
  show_status
}
...
run_manager() {
  ...
  echo "=== OpenPI pi0.5 SO101 Stage 3.5 recovery-heavy training manager ==="
  ...
  for BATCH_SIZE in $BATCH_SIZES; do
    ...
    echo
    echo "=== Attempting Stage 3.5 training with batch_size=${BATCH_SIZE} ==="
...
if [[ "${_STAGE35_MANAGER:-0}" == "1" ]]; then
  run_manager
fi

mkdir -p "$LOG_DIR"
setsid env _STAGE35_MANAGER=1 \
...
echo "Started OpenPI Stage 3.5 recovery-heavy training manager."
echo "PID: $PID"
echo "Manager log: $MANAGER_LOG"
echo "Status: bash control_scripts/38_train_openpi_stage35_recovery_1x3.sh --status"
