...
# Build the OpenPI pi0.5 CUHK curated Stage 3 recovery aggregate dataset.
#
# Composition:
#   - Stage 2 main-disorder aggregate: 1x
#   - Curated recovery pool: 2x
#
# The curated recovery pool intentionally excludes the triple-disorder datasets.
# It contains pairwise swap recovery plus single-letter random recovery where
# the later letters are already correctly placed.
...
TARGET_REPO="${TARGET_REPO:-guanlin8/cuhksz_pi05_stage3_curated_swap_x2_20260505}"
FORCE_REBUILD="${FORCE_REBUILD:-0}"
RUN_STAMP="$(date +%Y%m%d_%H%M%S)"
LOG_ROOT="${LOG_ROOT:-/home/ubuntu/evo-rl-control/outputs/dataset_build_logs}"
LOG_FILE="${LOG_ROOT}/${RUN_STAMP}_stage3_curated_swap_x2.log"
...
echo "=== Build OpenPI Stage 3 curated swap x2 dataset ==="
...
TEMP_ROOT="${ROOT}/.tmp_stage3_curated_swap_x2_${RUN_STAMP}"
...
for repo in unique_repos:
    ...
    # Some source datasets use the modern task string as the dataframe index,
    # while older aggregate datasets have `task_index` as a column and a numeric
    # dataframe index. Normalize the temporary copies only, so source datasets
    # remain untouched.
...
echo
echo "Adding Stage 3 curated-swap provenance to info.json..."
...
info = json.loads(info_path.read_text())
info["stage3_curated_swap_x2_unified_prompt"] = (
    "Sort the visible CUHK letter blocks into the lower target slots in left-to-right order C, U, H, K."
)
info["stage3_curated_swap_x2_mix"] = {
    "stage2_replay_copies": 1,
    "curated_recovery_copies": 2,
    "stage2_frames_expected": 88302,
    "curated_recovery_frames_one_pass_expected": 112753,
    "total_frames_expected": 313808,
    "recovery_ratio_expected": 225506 / 313808,
    "excluded": [
        "cuhksz_recovery_triple_c_hku_20260504",
        "cuhksz_recovery_triple_h_u_kc_20260504",
        "cuhksz_recovery_triple_uk_h_c_20260504",
        "cuhksz_recovery_triple_uhc_k_20260504",
        "cuhksz_recovery_triple_kc_h_u_20260504",
        "cuhksz_recovery_random_c_with_no_uhk_20260503",
        "cuhksz_recovery_random_u_with_no_hk_20260504",
        "cuhksz_recovery_random_h_with_no_k_20260504",
    ],
}
info_path.write_text(json.dumps(info, indent=2) + "\n")
PY
